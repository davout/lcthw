...
int Command_fetch(apr_pool_t *p, const char *url, int fetch_only)
{
  ...
  if(apr_fnmatch(GIT_PATH, info.path, 0) == APR_SUCCESS) {
  ...
  } else if(apr_fnmatch(DEPENDS_PAT, info.path, 0) == APR_SUCCESS) {
    ...
    // this indicates that nothing has to be done
    return 0;
    ...
    check(rc == 0, "Failed to create build dir: %s", BUILD_DIR);

    rc = Shell_exec(TAR_SH, "FILE", TAR_BZ2_SRC, NULL);
    check(rc == 0, "Failed to untar: %s", TAR_BZ2_SRC);

  } else {
    sentinel("Don't know how to handle URL: %s", url);
  }

  // indicates that an install actually needs to run
  return 1;
    ...
    const char *make_opts, const char *install_opts)
{
  ...
  check(access(BUILD_DIR, X_OK | R_OK | W_OK) == 0,
      "Build directory does not exist: %s", BUILD_DIR);
  ...
  rc = DB_update(url);
  check(rc == 0, "Failed to update database with installed package: %s", url);
